...
class Folder extends BaseController
{
    ...
    public function Manage() {
        ...
        return view('Folder/Manage', $data);
    }
    ...
    public function ModifyFolder() {
        ...
        if (!$validation->check($state, 'required')) {
            $json_data = [
                'remark' => 'Please ensure that you have selected a state for the folder',
                'state'  => false
            ];
            ...
            if (!$validation->check($folder_id, 'required')) {
                $json_data = [
                    'remark' => 'Please ensure you have selected a folder to rename',
                    'status' => false
                ];
                return $this->response->setJSON($json_data);
            } else {
    ...
    public function RenameFolder() {
        ...
        if (!$validation->check(trim($new_folder_name), 'required|min_length[1]')) {
            $json_data = [
              'remark' => 'Please ensure you have provided a valid name for the folder',
              'status' => false
            ];
    ...
    public function CreateNewFolder() {
        ...
        if (!$validation->check(trim($folder_name), 'required|max_length[128]')) {
            $json_data = [
                'remark' => 'Please provide a valid name for the folder',
                'status' => false
            ];
            return $this->response->setJSON($json_data);
        } else {
